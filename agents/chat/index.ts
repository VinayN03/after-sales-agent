...
async function* streamAfterSales(
  ...
  // Prefer the stored copy: /approve-refund may have changed the status since
  // this conversation's state was saved, and a stale copy would overwrite it.
  let preloadedOrder = priorState?.orderId ? await getOrder(context, priorState.orderId) : null;
  preloadedOrder = preloadedOrder || priorState?.currentOrder || null;
  ...
  const input: Partial<AfterSalesStateType> = {
    ...(priorState || pendingAction ? {
      ...
      refundReason: priorState?.refundReason ?? "",
    } : {}),
  ...
  for await (const rawEvent of stream as AsyncIterable<unknown>) {
    ...
    if (mode === "custom") {
      if ((event as { type?: string })?.type === "agent_event") {
        yield sseEvent(event as Record<string, unknown>);
        continue;
      }
  ...
  await saveState(context, conversationId, {
    ...
    refundReason: lastState.refundReason,
    locale,
  });
