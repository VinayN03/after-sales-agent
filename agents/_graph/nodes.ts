...
import { runRefundPipeline } from "../_agents/pipeline";
import { ensureDemoOrders } from "../_agents/data";
  ...
  try {
    ...
    if (match) {
      ...
      return {
        ...
        // Remember why the customer wants a refund — it may arrive before the order ID does.
        ...(parsed.intent === "refund" ? { refundReason: state.userInput } : {}),
      };
    }
  } catch {}
...
export async function requestRefund(state: AfterSalesStateType, context: AgentContext, env: AgentEnv, runtime?: StreamRuntime) {
  const locale = (state.locale || "zh") as Locale;
  const sep = locale === "en" ? ", " : "、";
  ...
  // Multi-agent pipeline: specialists analyse the case, a deterministic decision engine picks the
  // approval route, and only then (autonomously or after a human decides) does anything execute.
  return runRefundPipeline({
    order,
    userMessage: state.refundReason || state.userInput,
    context,
    env,
